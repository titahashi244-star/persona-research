...
def analyze_with_gemini(company_name: str, person_name: str, department: str, research: dict) -> dict:
    ...
    prompt = f"""
あなたは一流のB2B営業コンサルタントです。「この会社のことを誰よりも理解している凄腕の営業」として、
DNPペルソナインサイトの商談前レポートを作成してください。
...
{{
  ...
  "proposal_angles": {{
    "summary": "DNPペルソナインサイトの提案切り口（4〜5文。{company_name}固有の課題に紐づけて）",
    "angles": [
      {{"angle": "提案切り口1", "reason": "なぜこの会社に刺さるか", "example": "具体的な活用イメージ"}},
      {{"angle": "提案切り口2", "reason": "なぜ刺さるか", "example": "活用イメージ"}},
      {{"angle": "提案切り口3", "reason": "なぜ刺さるか", "example": "活用イメージ"}}
    ],
    "key_message": "商談での一言キーメッセージ（30字程度）"
  }},
  "anticipated_qa": [
...
<div class="qa-a"><strong>なぜ刺さるか：</strong>{ang.get("reason","")}<br><strong>活用イメージ：</strong>{ang.get("example","")}</div>
    ...
    sections.append(render_section("💡", "DNPペルソナインサイト 提案切り口", "#e94560",
        f"<p>{a.get('summary','')}</p>{angles_html}"
